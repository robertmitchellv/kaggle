...
def main():
    ...
    # --> n_estimators = the number of trees in this forest, viz. 100 trees of forest
    # --> n_jobs set to -1 will use the number of cores present on your system.
    rf = RandomForestClassifier(n_estimators = 100, n_jobs = -1)
    # fit(X, y[, sample_weight]) = build a forest of tress from the training set (X, y)
    rf.fit(train, target)
    
    # predict_proba(X) predict class probabilities for X as list with 2501 elements
    predicted_probs = [x[1] for x in rf.predict_proba(test)]
    
    # pandas series = a one dimentional ndarray with axis labels on the previously predicted
    ...
    """
    # write predicted_probs to file with pandas method .to_csv()--add header for submission
    try:
        predicted_probs.to_csv('Data/submission.csv', header = True, index_label = ['MoleculeId', 
